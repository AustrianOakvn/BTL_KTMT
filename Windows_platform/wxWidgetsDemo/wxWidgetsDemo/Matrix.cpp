...
using std::ostream; using std::istream; using std::end;
...
//Matrix& Matrix::operator*=(const Matrix& m) {
//    // need to benchmark time
//    auto start = std::chrono::high_resolution_clock::now();
//    Matrix temp(rows_, m.cols_);
//    for (int i = 0; i < temp.rows_; ++i) {
//        for (int j = 0; j < temp.cols_; ++i) {
//            for (int k = 0; k < cols_; ++k) {
//                temp.p[i][j] += (p[i][k] * m.p[k][j]);
//            }
//        }
...
std::string Matrix::get_presentation() {
    std::string ret;
    for (int i = 0; i < rows_; ++i) {
        for (int j = 0; j < cols_; ++j) {
            ret.append(std::to_string(p[i][j]));
            ret.append(" ");
        }
        ret.append("\n");
    }
...
//Matrix Matrix::expHelper(Matrix& m, int num)
//{
//    if (num == 0) {
